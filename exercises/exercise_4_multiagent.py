...
def check_routing(state: State) -> list[Send]:
    ...
    # ✅ Nhiệm vụ 2: routing cho privacy_agent
...
def aggregate_results(state: State) -> dict:
    ...
    # ✅ Nhiệm vụ 4: thêm privacy_analysis vào sections
...
def build_graph() -> StateGraph:
    ...
    # ✅ Nhiệm vụ 3: thêm privacy_agent node
    graph.add_node("privacy_agent", privacy_agent)
    graph.add_node("aggregate_results", aggregate_results)

    # Define edges
    graph.add_edge(START, "law_agent")
    # ⚠️ check_routing là HÀM conditional-edge (trả list[Send]), KHÔNG phải node.
    # Scaffold gốc add_node("check_routing") gây InvalidUpdateError vì node phải trả dict.
    ...
    # ✅ Nhiệm vụ 3: edge từ privacy_agent đến aggregate_results
