...
async def main():
    ...
    llm = get_llm()
    # Bài Tập 3.2: đổi debug=True để xem chi tiết reasoning của agent ([values]/[updates] mỗi bước).
    # Lưu ý: create_react_agent của LangGraph dùng `debug`, KHÔNG phải `verbose` (verbose gây TypeError).
    graph = create_react_agent(model=llm, tools=TOOLS, prompt=SYSTEM_PROMPT, debug=False)
