...
| LLM provider | [Google Gemini](https://ai.google.dev/gemini-api/docs/openai) (OpenAI-compatible endpoint) |
...
cp .env.example .env
# Edit .env with your Gemini API key (GEMINI_API_KEY)
```
...
│   ├── llm.py                 # get_llm() → ChatOpenAI via Gemini (OpenAI-compatible)
...
|---|---|---|
| `GEMINI_API_KEY` | Your Google Gemini API key (from https://aistudio.google.com/apikey) | (required) |
| `GEMINI_MODEL` | Model identifier | `gemini-2.0-flash` |
| `GEMINI_MAX_TOKENS` | Max completion tokens (raise for `gemini-2.5-flash` thinking) | `1024` |
| `REGISTRY_URL` | Registry service URL | `http://localhost:10000` |

The model is swappable to any Gemini model (e.g., `gemini-2.5-flash`, `gemini-2.5-pro`). `get_llm()` uses Gemini's OpenAI-compatible endpoint, so switching providers means changing only `common/llm.py` + `.env`.
